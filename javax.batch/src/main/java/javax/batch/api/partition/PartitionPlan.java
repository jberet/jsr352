...
public interface PartitionPlan {
    ...
    /**
     * Specify whether or not to override the partition count from the previous
     * job execution. This applies only to step restart. The default is false.
     * <p>
     * When false is specified, the partition count from the previous job
     * execution is used and any new value set for partition count in the
     * current run is ignored. In addition, partition results from the previous
     * job execution are remembered, and only incomplete partitions are
     * reprocessed.
     * <p>
     * When true is specified, the partition count from the current run is used
     * and all results from past partitions are discarded. Any resource cleanup
     * or back out of work done in the previous run is the responsibility of the
     * application. The PartitionReducer artifact's rollbackPartitionedStep
     * method is invoked during restart before any partitions begin processing
     * to provide a cleanup hook.
     */
