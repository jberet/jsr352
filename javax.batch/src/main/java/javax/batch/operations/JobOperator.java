...
/**
 * JobOperator provide the interface for operating on batch jobs. Through the
 * JobOperator a program can start, stop, and restart jobs. It can additionally
 * inspect job history, to discover what jobs are currently running and what
 * jobs have previously run.
 * 
 * The JobOperator interface imposes no security constraints. However, the
 * implementer is free to limit JobOperator methods with a security scheme of
 * its choice. The implementer should terminate any method that is limited by
 * the security scheme with a JobSecurityException.
 * 
 */
...
public interface JobOperator {
    ...
    /**
     * Returns all JobInstances belonging to a job with a particular name in
     * reverse chronological order.
     * 
     * @param jobName
     *            specifies the job name.
     * @param start
     *            specifies the relative starting number (zero based) to return
     *            from the maximal list of job instances.
     * @param count
    ...
    /**
     * Returns execution ids for job instances with the specified name that have
     * running executions.
     * 
     * @param jobName
     *            specifies the job name.
     ...
     * key/value pairs specified when the instance was originally created by the
     * start method.
     * 
     * @param executionId
     *            specifies the execution from which to retrieve the parameters.
     ...
     * according to a means prescribed by the batch runtime implementation. This
     * may vary by implementation. If the Job XML is not found by that means,
     * then the batch runtime must search for the specified Job XML as a
     * resource from the META-INF/batch-jobs directory based on the current
     * class loader. Job XML files under META-INF/batch-jobs directory follow a
     * naming convention of "name".xml where "name" is the value of the
     * jobXMLName parameter (see below).
     * 
     * @param jobXMLName
     *            specifies the name of the Job XML describing the job.
     * @param jobParameters
     *            specifies the keyword/value pairs for attribute substitution
     *            in the Job XML.
     ...
     * 
     * @param executionId
     *            specifies the execution to to restart. This execution must be
     *            the most recent execution that ran.
    ...
    /**
     * Request a running job execution stops. This method notifies the job
     * execution to stop and then returns. The job execution normally stops and
     * does so asynchronously. Note JobOperator cannot guarantee the jobs stops:
     * it is possible a badly behaved batch application does not relinquish
     * control.
     * <p>
     ...
     * 
     * @param executionId
     *            specifies the job execution to stop. The job execution must be
     *            running.
     * @throws NoSuchJobExecutionException
    ...
    /**
     * Set batch status to ABANDONED. The instance must have no running
     * execution.
     * <p>
